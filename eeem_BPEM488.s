...
veBins:          rmb $288 ; 648 bytes for VE Table (% x 10)
verpmBins:       rmb $24  ; 36 bytes for VE Table RPM Bins (RPM)
vemapBins:       rmb $24  ; 36 bytes for VE Table MAP Bins (KpA x 10) 
barCorVals:      rmb $12  ; 18 bytes for barometric correction values (KpA x 10)
barCorDelta:     rmb $12  ; 18 bytes for barometric correction  (% x 10)
dwellvolts:      rmb $0C  ; 12 bytes for dwell battery correction (volts x 10)
dwellcorr:       rmb $0C  ; 12 bytes for dwell battery correction (% x 10)
tempTable1:      rmb $14  ; 20 bytes for table common temperature values (degrees C or F x 10)
tempTable2:      rmb $14  ; 20 bytes for table common temperature values (degrees C or F x 10)
matCorrTemps2:   rmb $12  ; 18 bytes for MAT correction temperature (degrees C or F x 10)
matCorrDelta2:   rmb $12  ; 18 bytes for MAT correction (% x 10)
primePWTable:    rmb $14  ; 20 bytes for priming pulse width (msec x 10)
crankPctTable:   rmb $14  ; 20 bytes for cranking pulsewidth adder (% x 10 of reqFuel)
asePctTable:     rmb $14  ; 20 bytes for after start enrichment adder (% x 10)
aseRevTable:     rmb $14  ; 20 bytes for after start enrichment time (engine revolutions)
wueBins:         rmb $14  ; 20 bytes for after warm up enrichment adder (% x 10)
TOEbins:         rmb $04  ; 4 bytes for TPS acceleration adder (%)
TOErates:        rmb $08  ; 8 bytes for TPS acceleration rate (%/Sec x 10)
DdBndBase:       rmb $01  ; 1 byte for injector deadband at 13.2V (mSec * 100)
DdBndCor:        rmb $01  ; 1 byte for injector deadband voltage correction (mSec/V x 100)
tpsThresh:       rmb $02  ; 2 bytes for Throttle Opening Enrichment threshold (TpsPctx10/100mS)
TOEtime:         rmb $01  ; 1 byte for Throttle Opening Enrich time in 100mS increments(mSx10)
ColdAdd:         rmb $01  ; 1 byte for Throttle Opening Enrichment cold temperature adder at -40F (%)
ColdMul:         rmb $01  ; 1 byte for Throttle Opening Enrichment multiplyer at -40F (%)
InjDelDegx10:    rmb $02  ; 2 bytes for Injection delay from trigger to start of injection (deg x 10)
OFCtps:          rmb $01  ; 1 byte for Overrun Fuel Cut min TpS%x10
OFCrpm:          rmb $02  ; 2 byte for Overrun Fuel Cut min RPM
OFCmap:          rmb $02  ; 2 bytes for Overrun Fuel Cut maximum manifold pressure permissive (KPAx10)
OFCdel:          rmb $01  ; 1 byte for Overrun Fuel Cut delay time (Sec x 10)
crankingRPM:     rmb $02  ; 2 bytes for crank/run transition (RPM)
floodClear:      rmb $02  ; 2 bytes for TPS position for flood clear (% x 10)
Stallcnt:        rmb $02  ; 2 bytes for no crank or stall condition counter (1mS increments)
tpsMin:          rmb $02  ; 2 bytes for TPS calibration closed throttle ADC
tpsMax:          rmb $02  ; 2 bytes for TPS calibration wide open throttle ADC(
reqFuel:         rmb $02  ; 2 bytes for Pulse width for 14.7 AFR @ 100% VE (mS x 10)
enginesize:      rmb $02  ; 2 bytes for displacement of two engine cylinders (for TS reqFuel calcs only)(cc)
staged_pri_size: rmb $01  ; 1 byte for flow rate of 1 injector (for TS reqFuel calcs only)(cc)
alternate:       rmb $01  ; 1 byte for injector staging bit field (for TS reqFuel calcs only)
nCylinders:      rmb $01  ; 1 byte for number of engine cylinders bit field (for TS reqFuel calcs only)
nInjectors:      rmb $01  ; 1 byte for number of injectors bit field (for TS reqFuel calcs only)
divider:         rmb $01  ; 1 byte for squirts per cycle bit field (for TS reqFuel calcs only)
...
stBins:     rmb $288 ; 648 bytes for ST Table
strpmBins:  rmb $24  ; 36 bytes for ST Table RPM Bins
stmapBins:  rmb $24  ; 36 bytes for ST Table MAP Bins
heton:      rmb $02  ; 2 bytes for High engine temperature alarm on set point (degF*10)
hetoff:     rmb $02  ; 2 bytes for High engine temperature alarm off set point (degF*10)
hotoF:      rmb $02  ; 2 bytes for High oil temperature alarm on set point (degF*10)
hotoff:     rmb $02  ; 2 bytes for High oil temperature alarm off set point (degF*10)
hfton:      rmb $02  ; 2 bytes for High fuel temperature alarm on set point (degF*10) 
hftoff:     rmb $02  ; 2 bytes for High fuel temperature alarm off set point (degF*10)
hegton:     rmb $02  ; 2 bytes for High exhaust gas temperature alarm on set point (degF*10)
hegtoff:    rmb $02  ; 2 bytes for High exhaust gas temperature alarm off set point (degF*10) 
lopon:      rmb $02  ; 2 bytes for Low engine oil pressure alarm on set point (psi*10)
lopoff:     rmb $02  ; 2 bytes for Low oil engine pressure alarm off set point (psi*10)
hfpon:      rmb $02  ; 2 bytes for High fuel pressure alarm on set point (psi*10)
hfpoff:     rmb $02  ; 2 bytes for High fuel pressure alarm off set point (psi*10)
lfpon:      rmb $02  ; 2 bytes for Low fuel pressure alarm on set point (psi*10)
lfpoff:     rmb $02  ; 2 bytes for Low fuel pressure alarm off set point (psi*10)
InjPrFlo:   rmb $02  ; 2 bytes for Pair of injectors flow rate (L/hr x 100)
...
afrBins:    rmb $288 ; 648 bytes for AFR Table (0)
afrrpmBins: rmb $24  ; 36 bytes for AFR Table RPM Bins (648)
afrmapBins: rmb $24  ; 36 bytes for AFR Table MAP Bins (684)
